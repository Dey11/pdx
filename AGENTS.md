# NoteFormula — AGENTS.md

NoteFormula is an AI study-material generator for students. It is one deployable repository containing a Next.js web application, a BullMQ generation worker, and a Redis service; PostgreSQL and object storage are managed externally.
...
- Keep orchestration in route handlers and worker entry points thin. Put provider-specific AI, storage, auth, email, and payment behavior behind the existing `src/lib` or `worker/src` boundaries.
...
- **Compose stack**: The production `redis`, `web`, and `worker` services described by `docker-compose.yml`.
...
- Treat AI model selection as configuration through `AI_GENERATION_MODELS`; avoid hard-coding deployment-specific provider choices.
...
- There is currently no automated test suite. For meaningful behavior changes, add focused tests where practical and run lint, type checking, the relevant build, and targeted manual checks.
