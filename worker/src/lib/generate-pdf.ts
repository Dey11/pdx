...
const getTitleFromMarkdown = (markdown: string) => {
  ...
  return (
    markdown
      ...
      .find(Boolean)
      ?.replace(/^#+\s+/, "")
      .slice(0, 90) || "NoteFormula Study Material"
  );
};
    ...
    await page.pdf({
      ...
      footerTemplate: `
        <div style="width:100%;font-size:9px;color:#6b7280;padding:0 32px;display:flex;justify-content:space-between;">
          <span>NoteFormula Study Material</span>
...
const buildHtml = (markdown: string, options: PdfRenderOptions) => {
  ...
  const subtitle = options.subtitle || "Generated by NoteFormula";
...
<body>
  <section class="cover-page">
    <div>
      <div class="cover-eyebrow">NoteFormula Study Material</div>
      <h1 class="cover-title">${escapeHtml(title)}</h1>
      <p class="cover-subtitle">${escapeHtml(subtitle)}</p>
      <div class="cover-meta">
        <span>Generated ${escapeHtml(generatedAt)}</span>
        <span>noteformula.com</span>
      </div>
    </div>
  </section>
  <main>
